...
void TexturedTerrainApplication::Render()
{
    ...
    // Terrain patches
    DrawObject(m_terrainPatch, *m_terrainMaterial_01, glm::scale(glm::vec3(10.0f))); // bottom right

    // (todo) 04.2: Add more patches here
    DrawObject(m_terrainPatch, *m_terrainMaterial_02, glm::translate(glm::vec3(-10, 0, 0)) * glm::scale(glm::vec3(10.0f))); // bottom left
    DrawObject(m_terrainPatch, *m_terrainMaterial_03, glm::translate(glm::vec3(0, 0, -10)) * glm::scale(glm::vec3(10.0f))); // top right
    DrawObject(m_terrainPatch, *m_terrainMaterial_04, glm::translate(glm::vec3(-10, 0, -10)) * glm::scale(glm::vec3(10.0f))); // top left

    // Water patches
    // (todo) 04.5: Add water planes
    DrawObject(m_terrainPatch, *m_waterMaterial, glm::translate(glm::vec3(0, -1.5, 0)) * glm::scale(glm::vec3(10.0f))); // bottom right
    DrawObject(m_terrainPatch, *m_waterMaterial, glm::translate(glm::vec3(-10, -1.5, 0)) * glm::scale(glm::vec3(10.0f))); // bottom left
    DrawObject(m_terrainPatch, *m_waterMaterial, glm::translate(glm::vec3(0, -1.5, -10)) * glm::scale(glm::vec3(10.0f))); // top right
    DrawObject(m_terrainPatch, *m_waterMaterial, glm::translate(glm::vec3(-10, -1.5, -10)) * glm::scale(glm::vec3(10.0f))); // top left
}
...
void TexturedTerrainApplication::InitializeTextures()
{
    ...
    m_heightmapTexture_01 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(0, 0));
    m_heightmapTexture_02 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(-1, 0));
    m_heightmapTexture_03 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(0, -1));
    m_heightmapTexture_04 = CreateHeightMap(m_gridX, m_gridY, glm::ivec2(-1, -1));
...
void TexturedTerrainApplication::InitializeMaterials()
{
    ...
    m_terrainMaterial_01->SetUniformValue("GrassTexture", m_grassTexture);
    m_terrainMaterial_01->SetUniformValue("SnowTexture", m_snowTexture);
    m_terrainMaterial_01->SetUniformValue("RockTexture", m_rockTexture);
    m_terrainMaterial_01->SetUniformValue("DirtTexture", m_dirtTexture);
    m_terrainMaterial_01->SetUniformValue("ColorTextureScale", glm::vec2(0.04));
    m_terrainMaterial_01->SetUniformValue("Heightmap", m_heightmapTexture_01);
    m_terrainMaterial_01->SetUniformValue("Range_01", glm::vec2(-0.2f, 0.0f));
    m_terrainMaterial_01->SetUniformValue("Range_02", glm::vec2(0.1f, 0.2f));
    m_terrainMaterial_01->SetUniformValue("Range_03", glm::vec2(0.25f, 0.3f));

    m_terrainMaterial_02 = std::make_shared<Material>(*m_terrainMaterial_01);
    m_terrainMaterial_02->SetUniformValue("Heightmap", m_heightmapTexture_02);

    m_terrainMaterial_03 = std::make_shared<Material>(*m_terrainMaterial_01);
    m_terrainMaterial_03->SetUniformValue("Heightmap", m_heightmapTexture_03);

    m_terrainMaterial_04 = std::make_shared<Material>(*m_terrainMaterial_01);
    m_terrainMaterial_04->SetUniformValue("Heightmap", m_heightmapTexture_04);
...
std::shared_ptr<Texture2DObject> TexturedTerrainApplication::CreateHeightMap(unsigned int width, unsigned int height, glm::ivec2 coords)
{
    ...
    for (unsigned int j = 0; j < height; ++j)
    {
        for (unsigned int i = 0; i < width; ++i)
        {
            ...
            float x = static_cast<float>(i) / (width - 1) + coords[0];
            float y = static_cast<float>(j) / (height - 1) + coords[1];
            float z = stb_perlin_fbm_noise3(x, y, 0.0f, 1.9f, 0.5f, 8) * 0.5f;
