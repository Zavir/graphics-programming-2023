...
vec3 GetSpecularReflection(vec3 lightVector, vec3 viewVector, vec3 normalVector) {
	...
	return SpecularReflection * LightColor * pow(max(dot(normalVector, halfVector), 0.0f), SpecularExponent);
}

vec3 GetDiffuseReflection(vec3 lightVector, vec3 normalVector, vec3 objectColor) {
	return DiffuseReflection * LightColor * objectColor *  max(dot(normalVector, lightVector), 0.0f);
}
